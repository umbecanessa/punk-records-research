...
from greenfield.renderer.phrasing import sample_reference
...
class ParaphraseRenderDataset(Dataset):
    """E11b — random phrasing variant per slot/value (multi-template answers)."""
    ...
    def _build(self) -> list[tuple[int, str, str]]:
        ...
        while len(rows) < self.size:
            world = sample_world(rng, num_facts=1 + rng.randint(0, 2))
            for key, val in world.facts.items():
                ref = sample_reference(rng, key, str(val))
                rows.append((SLOT_TO_ID.get(key, 0), str(val), ref))
                if len(rows) >= self.size:
                    break
        return rows
